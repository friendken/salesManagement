...
//                    $('#tabel_product_type').dataTable();
        ...
        .controller('warehouseSaleController', ['$scope', '$http', '$stateParams', 'showAlert', 'renderSelect','$filter', function ($scope, $http, $stateParams, showAlert, renderSelect,$filter) {
                $scope.searchCustomer = '';
                if ($stateParams.type === 'wholesale') {
                ...
                $scope.printMe = function () {
                    ...
                    var html = '';
                    var i = 1;
                    var table = $filter('filter')($scope.wholesale.buy_price, function (item) {
                        var product = $filter('filter')($scope.products, function (actual, expected) {
                            return angular.equals(actual.product_id, item.product_id)
                        });
                        html += '<tr><td>' + i + '</td>';
